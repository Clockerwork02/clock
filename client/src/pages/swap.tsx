                          ...
                          <Button
                            onClick={async () => {
                              ...
                              try {
                                ...
                                // Handle specific rate limit errors and set appropriate error message
                                if (error.message?.includes('rate limit') || 
                                    error.message?.includes('upstream') || 
                                    error.code === -32603) {
                                  setSwapError(`RPC rate limited. Click retry to use backup endpoints. Error: ${error.message}`);
                                } else if (error.message?.includes('insufficient funds')) {
                                  setSwapError(`Insufficient balance for transaction and gas fees. Error: ${error.message}`);
                                } else if (error.message?.includes('user denied') || error.code === 4001) {
                                  setSwapError(`Transaction cancelled by user. Error: ${error.message}`);
                                } else {
