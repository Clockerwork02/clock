...
export function useDirectBrowserWallet() {
  ...
  const connect = useCallback(async (walletProvider?: any) => {
    ...
    try {
      ...
      // Use provided wallet provider or detect automatically
      let ethereum = walletProvider || window.ethereum;
      ...
      if (!ethereum) {
        ...
        if (window.okxwallet) {
          ...
          throw new Error('No wallet found. Please install MetaMask, OKX, or Trust Wallet.');
        }
      } else {
